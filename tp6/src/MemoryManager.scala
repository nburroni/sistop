/**
  ...
  */
case class MemoryManager(virtualMemory: VirtualMemory, memory: Memory, processes: List[Process], pageReplacementAlgorithm: PageReplacementAlgorithm){
  ...
  def getPage(processId: Int, access: Access) = {
    ...
    if (fault) processPageFaults += (processId -> processPageFaults.getOrElse(processId,0))
  }
}
